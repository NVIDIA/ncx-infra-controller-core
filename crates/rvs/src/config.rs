...
/// BMMC (Carbide API) connection settings.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct BmmcConfig {
...
mod tests {
    ...
    fn test_defaults_are_valid() {
        ...
        assert_eq!(config.metrics_endpoint, "[::]:9019".parse::<SocketAddr>().unwrap());
        assert_eq!(config.bmmc.rpc_timeout_secs, 30);
    ...
    fn test_load_without_file_uses_defaults() {
        ...
        assert_eq!(config.bmmc.url, "https://carbide-api.forge-system.svc.cluster.local:1079");
        assert_eq!(config.tls.root_cafile_path, "/var/run/secrets/spiffe.io/ca.crt");
    }
