...
pub async fn handle_error(
    ...
    if let Some(scope) = &config.maintenance_requested {
        if scope.is_full_rack() {
            tracing::info!(
                "Rack {} on-demand maintenance requested from Error state (full rack), transitioning to Maintenance",
                id
            );
        } else {
            tracing::info!(
                "Rack {} on-demand maintenance requested from Error state (partial: {} machines, {} switches, {} power shelves), transitioning to Maintenance",
                id,
