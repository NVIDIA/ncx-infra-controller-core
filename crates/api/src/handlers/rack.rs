...
use model::rack::{MaintenanceScope, RackState};
...
pub(crate) async fn on_demand_rack_maintenance(
    ...
    if !matches!(*rack.controller_state, RackState::Ready | RackState::Error { .. }) {
        return Err(CarbideError::InvalidArgument(format!(
    ...
    let scope = MaintenanceScope {
        ...
        power_shelf_ids: req
            ...
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| {
                CarbideError::InvalidArgument(format!("Invalid power_shelf_id: {e}"))
            })?,
    };
    ...
    tracing::info!(
        "On-demand maintenance scheduled for rack {}",
        rack_id,
    );
