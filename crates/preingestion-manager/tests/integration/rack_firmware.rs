...
async fn compute_submission_is_device_scoped_and_resumes_polling(
    ...
    let mut submission = MockRmsApi::firmware_object_apply_ok(&bmc_mac.to_string(), "compute-job");
    // A returned job remains authoritative even when the batch summary is not
    // successful; RMS may already be running the accepted operation.
    let batch = submission.response.as_mut().unwrap();
    ...
    batch.node_results[0].error_message = "partial batch response".to_string();
