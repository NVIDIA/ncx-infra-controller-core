...
func TestIPBlockHandler_Delete(t *testing.T) {
	...
	t.Run("rejects an allocation that commits while deletion waits", func(t *testing.T) {
		...
		ec.SetParamValues(ipOrg1, raceRoot.ID.String())
		ec.Set("user", user)
		...
		handlerStarted = true
		go func() {
			handlerDone <- (DeleteIPBlockHandler{
				dbSession: dbSession,
				tc:        tempClient,
				cfg:       cfg,
			}).Handle(ec)
		...
		require.NotNil(t, ipamer.PrefixFrom(ctx, raceRootPrefix.Cidr))
		...
		raceCases := []struct {
			...
			},
			{
				...
				run: func(t *testing.T, fixture raceFixture) {
					...
					deleted, _, err := cdbm.NewIPBlockDAO(dbSession).GetAll(
						ctx,
						nil,
						cdbm.IPBlockFilterInput{
							IPBlockIDs:     []uuid.UUID{fixture.ipBlock.ID},
							IncludeDeleted: true,
						},
					...
					assert.Nil(t, deleted[0].SitePrefixID)
