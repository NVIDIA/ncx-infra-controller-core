...
func TestAllocationHandler_Create(t *testing.T) {
	...
	t.Run("reports an allocation parent deleted before its row lock", func(t *testing.T) {
		...
		_, allocationCount, err := cdbm.NewAllocationDAO(dbSession).GetAll(
			ctx,
			nil,
		...
		_, constraintCount, err := cdbm.NewAllocationConstraintDAO(dbSession).GetAll(
			ctx,
			nil,
			cdbm.AllocationConstraintFilterInput{
				ResourceType:    cutil.GetPtr(cdbm.AllocationResourceTypeIPBlock),
				ResourceTypeIDs: []uuid.UUID{raceRoot.ID},
			},
			cdbp.PageInput{},
			nil,
		)
		...
		_, childCount, err := ipbDAO.GetAll(
			ctx,
			nil,
			cdbm.IPBlockFilterInput{
				Names:     []string{raceName},
				TenantIDs: []uuid.UUID{tenant1.ID},
				SiteIDs:   []uuid.UUID{site.ID},
			},
			cdbp.PageInput{},
