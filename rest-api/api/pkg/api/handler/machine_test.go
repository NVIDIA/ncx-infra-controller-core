...
func TestMachineHandler_Delete(t *testing.T) {
	...
	forceTests := []struct {
		...
		{name: "missing Machine requires regular deletion even with all overrides", query: "/?force=true&allowDeleteWithInstanceType=true&allowDeleteWithInstance=true&allowDeleteWithAllocation=true", isMissingOnSite: true, hasAttachedInstance: true, allocationCount: 1, wantStatus: http.StatusBadRequest, wantMessage: "Machine is already missing on Site and cannot be force deleted. Please try regular deletion without force=true"},
