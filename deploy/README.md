...
- `deploy/files/nico-api/nico-api-site-config.toml` – set site identifiers (`ENVIRONMENT_NAME`, `SITE_DOMAIN_NAME`), admin network prefixes, service VIPs (API, DHCP, DNS, PXE, SSH console, NGINX/proxy, Unbound), tenant overlay prefixes, and IPMI pools/names for controllers and managed hosts. Ensure service VIPs come from your chosen /27 (or equivalent) VIP pool and that IPMI networks have unique names. For seed network definitions, an IPv4 prefix requires a gateway; see [Seed Network Configuration](../docs/provisioning/ip-and-network-configuration.md#seed-network-configuration) for IPv6 fields and gateway omission rules.
