...
combinations and gateway omission rules, see
[Seed Network Configuration](#seed-network-configuration).
...
`[networks.<name>]` blocks. Each segment carries its own prefixes, MTU, and
...
The `[networks.<name>]` blocks in `siteConfig` or `initial_objects_file` seed
network segments. Each definition supports IPv4-only, dual-stack, or IPv6-only
prefixes. DPU provisioning requires an `admin` segment with an IPv4 prefix and
gateway. An IPv6-only admin definition can be seeded, but DPU network
configuration fails because it has no IPv4 admin prefix.
...
| `prefix_v6` | Optional IPv6 CIDR. Omit it for single-stack definitions. Set it only alongside an IPv4 `prefix` to create a dual-stack segment. It cannot replace `prefix` or accompany an IPv6 `prefix`. |
| `gateway` | Required when `prefix` is IPv4, including dual-stack definitions. Supply an IPv4 address within that prefix; it does not have to equal the relay's `giaddr`. IPv6-only definitions may omit it. This field does not configure an IPv6 gateway. |
| `dhcpv6_link_address` | Optional IPv6 address, without a CIDR suffix, used to match a DHCPv6 relay's link-address. Requires an IPv6 prefix. It may lie outside that prefix but must be unique across network prefixes. Omission leaves no explicit link-address match; it does not infer a gateway or relay address. |
...
NICo's seed validation does not enforce this IPv6-specific minimum.
...
Gateway omission for IPv6-only seeds requires a binary containing
[IPv6-only seed gateway support](https://github.com/NVIDIA/infra-controller/pull/5952),
tracked in [IPv6-only seed networks without a dummy gateway](https://github.com/NVIDIA/infra-controller/issues/5401).
Existing IPv6-only definitions may keep a supplied gateway of either address
family. NICo accepts that value for compatibility and retains it in the stored
seed definition, but does not attach it to the IPv6 network prefix.

Seed declarations do not update existing network segments. Removing a supplied
gateway from an already seeded IPv6-only definition produces a configuration drift
warning; it does not rewrite the segment or its stored seed definition. Before
creating IPv6-only seeds without a gateway, check your rollback requirements:
binaries predating this support cannot read stored seed definitions without a
gateway.
...
- [ ] `siteConfig` network definitions follow the [seed field requirements](#seed-network-configuration), including a gateway whenever an IPv4 prefix is present.
